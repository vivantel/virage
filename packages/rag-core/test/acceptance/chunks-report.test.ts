...
describe('rag-update chunks report', () => {
  beforeAll(() => {
  ...
  it('exits 0 and shows cohesion stats', () => {
    const result = runCLI(dir, 'chunks', 'report', '--file', join(dir, 'rag-test', 'chunks.json'));
  ...
  it('exits non-zero for a missing chunks file', () => {
    const result = runCLI(dir, 'chunks', 'report', '--file', join(dir, 'nonexistent.json'));
