...
export const MONOREPO = fileURLToPath(new URL('../../../../../', import.meta.url));
export const CLI = join(MONOREPO, 'packages/rag-core/dist/bin/rag-update.js');
export const STORE_PKG = '@vivantel/rag-store-test';
...
function buildConfig(env: TestEnv): Record<string, unknown> {
  return {
    chunkers: [{ patterns: ['skills/*.md', '*.md'], strategy: 'markdownHeaders' }],
    embedder: {
      package: '@vivantel/rag-embedder-fastembed',
      config: { model: 'fast-bge-small-en-v1.5', dimensions: 384, cacheDir: env.cacheDir },
    },
    vectorStore: {
      package: STORE_PKG,
...
export function setupEnv(): TestEnv {
  ...
  if (!process.env.E2E_CLONE_DIR) {
    execSync(`git clone --depth=1 git@github.com:OpenHands/OpenHands.git ${cloneDir}`, {
      stdio: 'inherit',
    });
  }
