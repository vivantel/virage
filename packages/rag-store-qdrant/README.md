# @vivantel/rag-store-qdrant

Qdrant vector store for [`@vivantel/rag-core`](../rag-core/README.md). Works with both local Qdrant instances and Qdrant Cloud (SaaS).

## Installation

```bash
npm install @vivantel/rag-store-qdrant @vivantel/rag-core
```
...
```typescript
import { QdrantVectorStore } from "@vivantel/rag-store-qdrant";
...
### Index stats (`rag-update store stats`)
...
### Query performance (`rag-update store perf --timeframe 24`)
