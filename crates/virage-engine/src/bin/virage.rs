...
fn virage_render_config() -> inquire::ui::RenderConfig<'static> {
    ...
    loop {
        match step {
            ...
            0 => {
                ...
                match select_step("Telemetry endpoint", &choices, 0)? {
                    None => {
                        out.info("Cancelled.");
                        return Ok(());
                    }
            ...
            2 => {
                ...
                match select_step("Enable tier-2 usage telemetry?", &choices, 1)? {
                    None => {
            ...
            3 => {
                ...
                match select_step("Sampling rate", &choices, 1)? {
                    None => {
                        step = 2;
                        continue;
                    }
...
async fn cmd_bench_index(
    ...
    let result = virage_engine::bench::BenchResult::new(
        corpus_path.clone(),
    ...
    match format {
        ...
        OutputFormat::Markdown => {
            out.data_line(&virage_engine::bench::report::format_markdown(&cmp))
        }
        _ => println!("{}", virage_engine::bench::report::format_console(&cmp)),
    }
...
async fn cmd_serve(args: &ServeArgs, config: &str) -> anyhow::Result<()> {
    ...
    loop {
        ...
        let result: Result<serde_json::Value, serde_json::Value> = match method {
            ...
            "tools/list" => Ok(virage_engine::mcp::tools_list()),
            "tools/call" => {
                let name = request
                    .pointer("/params/name")
                    .and_then(|v| v.as_str())
                ...
                let call_args = request
                    ...
                    .cloned()
                    .unwrap_or_default();
                virage_engine::mcp::tool_call(&name, &call_args, &embedder, &store, &db_path, None)
                    .await
            }
...
async fn main() {
    ...
    let result = match cli.command {
        ...
        Some(Commands::Bench(args)) => cmd_bench(args, cli.verbose, format, config).await,
