...
**Intermediate artifacts**: `docs/rag/chunks.json` and `docs/rag/embeddings.json` are gitignored — they are generated by the pipeline and cached in CI via `actions/cache`. `rag.config.ci.json` at the repo root is the CI-specific config (FastEmbed embedder + Postgres vector store) used by `.github/workflows/virage-update.yaml` — a standalone workflow that triggers on master pushes and installs published packages from npmjs rather than building from source.
