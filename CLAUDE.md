...
**Intermediate artifacts**: `docs/rag/chunks.json` and `docs/rag/embeddings.json` are gitignored — they are generated by the pipeline and cached in CI via `actions/cache`. `rag.config.ci.ts` at the repo root is the CI-specific config (GitHub Models embedder + Supabase vector store) used by the `rag-update` job in `.github/workflows/ci.yaml`.
