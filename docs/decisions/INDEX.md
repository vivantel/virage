...
| ADR-054 | Multi-worker pipeline concurrency via bounded tokio channels | Superseded | Superseded by ADR-057. Bounded channels alone don't prevent OOM under shared-host memory pressure. | [ADR-054](./ADR-054-pipeline-concurrency.md) |
| ADR-055 | Wire format-specific chunker routing into the live indexing pipeline | Accepted | `worker::process_item` routes bytes to the matching built-in/WASM chunker instead of always using `raw_bytes_to_doc` | [ADR-055](./ADR-055-wire-chunker-routing-into-pipeline.md) |
| ADR-056 | Structured logging via tracing + tracing-subscriber | Accepted | CE-scope structured logging: stderr + optional append-only file, no remote transport, no rotation | [ADR-056](./ADR-056-tracing-adoption.md) |
| ADR-057 | Streaming pipeline + resource-aware concurrency | Accepted | `ConcurrencyStrategy` trait (`FixedWorkers`/`RamSampling`); streaming chunk→embed→upload; bounded ORT session footprint | [ADR-057](./ADR-057-streaming-resource-aware-indexing.md) |
