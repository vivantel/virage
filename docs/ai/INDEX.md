...
- **Vector store is the authoritative change-detection source**: `Orchestrator.previousState` is `new Map([...fileRevisions, ...vectorStoreState])`. `vectorStoreState` (from the vector store) wins on any overlap. `fileRevisions` (from `VirageDb.getFileRevisions()`, `file_revisions` table only — no `chunks` backward-compat) supplements for zero-chunk files that never enter the vector store (ADR-033). Never merge `db.getFileStates()` (includes `chunks` table with legacy Windows backslash entries) into `previousState`
- **Chunk count denominator uses `embedTotal`, never extrapolation**: `embedTotal` starts at `initialPendingEmbed` and grows by `chunksToEmbed.length` per file — it equals `embedDone` after the final flush. Do not replace this with `chunksGenerated * toFiles / filesProcessed`; that formula overestimates on file 1 and the `PipelineRenderer` ratchet locks in the spike. Floor-of-1 (`Math.max(_, 1)`) must not be added to `sharedTotal`; zero-chunk runs should show `—`, not `0/1`
