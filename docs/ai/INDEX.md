...
- **Vector store is the sole change-detection source**: `Orchestrator.previousState` is built from `vectorStoreState` only (not from `db.getFileStates()`). `VirageDb` is a transient embedding buffer; SQLite file paths must never be merged into `previousState`. The authoritative intent is expressed in the comment above `vectorStore.initialize()` in `orchestrator.ts`
